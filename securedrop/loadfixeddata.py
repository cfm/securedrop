    ...
    for source_data in sources_data:
        ...
        for item_data in source_data["items"]:
            ...
            if kind == "message":
                ...
                # Create filename for reply
                fname = f"{source.interaction_count}-{source.journalist_filename}-reply.gpg"

                # Create encrypted reply file using real SecureDrop encryption
                encryption_mgr.encrypt_journalist_reply(
                    for_source=source,
                    reply_in=content,
                    encrypted_reply_path_out=Path(storage.path(source.filesystem_id, fname)),
                )

                # Create reply using constructor
...
def import_seen_records(
    ...
    # Process seen records from unified items list
    for source_data in sources_data:
        for item_data in source_data["items"]:
            uuid = item_data["uuid"]
            kind = item_data["kind"]
...
def load_fixed_data(yaml_path: Path) -> None:
    ...
    with app.app_context():
        ...
        uuid_to_submission, uuid_to_reply = import_submissions_and_replies(
            data["sources"], uuid_to_source, uuid_to_journalist
        )
...
if __name__ == "__main__":  # pragma: no cover
    ...
    load_fixed_data(args.yaml_path)
